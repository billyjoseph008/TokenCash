...
class QRCodeEncoder(
    ...
    private fun encodeQRCodeContents(
        data: String?,
        bundle: Bundle?,
        type: String
    ) {
        ...
        if (type == Contents.Type.TEXT) {
            ...
            if (bundle != null) {
                ...
                val name =
                    trim(bundle.getString(ContactsContract.Intents.Insert.NAME))
                if (name != null) {
                ...
                val address =
                    trim(bundle.getString(ContactsContract.Intents.Insert.POSTAL))
                if (address != null) {
                ...
                val uniquePhones: MutableCollection<String> =
                    HashSet<String>(Contents.PHONE_KEYS.size)
                for (x in 0 until Contents.PHONE_KEYS.size) {
                    val phone =
                        trim(bundle.getString(Contents.PHONE_KEYS[x]))
                    if (phone != null) {
                ...
                val uniqueEmails: MutableCollection<String> =
                    HashSet<String>(Contents.EMAIL_KEYS.size)
                for (x in 0 until Contents.EMAIL_KEYS.size) {
                    val email =
                        trim(bundle.getString(Contents.EMAIL_KEYS[x]))
                    if (email != null) {
                ...
                val url =
                    trim(bundle.getString(Contents.URL_KEY))
                if (url != null) {
                ...
                val note =
                    trim(bundle.getString(Contents.NOTE_KEY))
                if (note != null) {
            ...
            if (bundle != null) {
                ...
                val latitude =
                    bundle.getFloat("LAT", Float.MAX_VALUE)
                val longitude =
                    bundle.getFloat("LONG", Float.MAX_VALUE)
